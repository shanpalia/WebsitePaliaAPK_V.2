...
public class PaliaDownloaderPlugin extends Plugin {
    ...
    public void download(PluginCall call) {
        ...
        if (!sanitizedFilename.toLowerCase().endsWith(".apk")) sanitizedFilename += ".apk";
    ...
    private void performDownload(long id, String url, String filename, long expectedTotal) {
        ...
        try {
            ...
            long downloaded = 0L;
            long lastEvent = 0L;
            emitProgress(id, 1, 0L, total, filename);
            ...
            emitProgress(id, 8, actualSize, total > 0 ? total : actualSize, filename);
            ...
            error.put("error", e.getMessage() == null ? "Download failed" : e.getMessage());
            mainHandler.post(() -> notifyListeners("downloadProgress", error));
        } finally {
    ...
    private void emitProgress(long id, int status, long bytes, long total, String filename) {
        mainHandler.post(() -> {
    ...
    public void getProgress(PluginCall call) {
        ...
        File finalFile = filename.isEmpty() ? null : new File(downloadDirectory(), filename);
        File partFile = filename.isEmpty() ? null : new File(downloadDirectory(), filename + ".part");
